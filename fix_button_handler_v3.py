"""Reconstruct button_handler in alerts/commands.py to fix corruption and add missing handlers (v3)."""
...
if start_idx != -1:
    ...
    for i in range(start_idx + 1, len(lines)):
        if "async def set_tp_cmd" in line:
            end_idx = i
            break
    
    if end_idx == -1:
        print("Could not find set_tp_cmd to mark end of button_handler.")
        # Fallback: look for any async def that is not indented
        for i in range(start_idx + 1, len(lines)):
             if line.startswith("async def ") and not line.startswith("    "):
                 end_idx = i
                 break
    ...
    print("Successfully reconstructed button_handler (v3)!")
else:
