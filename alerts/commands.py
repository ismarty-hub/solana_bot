...
async def setalerts_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    if not chosen:
        ...
        await update.message.reply_html(
            ...
            f"⚠️™ï¸ <b>Configure Alert Grades</b>\n\n"
    ...
    if success:
        ...
        await update.message.reply_text("Œ Failed to save preferences. Please try again.")
...
async def performance_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager, portfolio_manager: PortfolioManager):
    ...
    try:
        ...
        logger.error(f"Error calculating PnL for performance report: {e}")


        pnl_data = {"total_unrealized_usd": 0.0, "total_cost_basis": 0.0}





    # Starting capital vs current total equity
    ...
    total_unrealized_pnl = pnl_data.get('total_unrealized_usd', 0.0)


    # Invested is the cost basis of open positions


    total_cost_basis = pnl_data.get('total_cost_basis', 0.0)


    


    # Total Equity (Market Value) = Cash + Cost Basis + Unrealized P/L


    total_value = current_capital + total_cost_basis + total_unrealized_pnl
    ...
    msg = (
        ...
        f"• Cash Balance: ${current_capital:,.2f}\n"


        f"• Open Positions (Cost): ${total_cost_basis:,.2f}\n"


        f"• Unrealized P/L: <b>${total_unrealized_pnl:+,.2f}</b>\n\n"


        


        f"<b>📈 Trade Statistics (Closed):</b>\n"
...
def _format_prediction_result(mint: str, data: dict) -> str:
    ...
    try:
        ...
        return f"Œ Error formatting prediction for <code>{mint}</code>."
...
async def predict_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    try:
        ...
        await loading_msg.edit_text(f"Œ <b>Request Timed Out</b>\n\nThe prediction service is taking too long to respond.")
        ...
        await loading_msg.edit_text(f"Œ <b>Connection Error</b>\n\nFailed to connect to the prediction service.")
        ...
        await loading_msg.edit_text(f"Œ <b>An Unexpected Error Occurred</b>\n\nPlease try again later.")
...
async def predict_batch_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    if len(mints) > 10:
        ...
        await update.message.reply_html("Œ <b>Error:</b> Maximum of 10 tokens per batch request.")
    ...
    try:
        ...
        await loading_msg.edit_text(
            ...
            "Œ <b>An Unexpected Error Occurred</b>\n\n"
...
async def set_tp_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):
    ...
    if val not in ["median", "mean", "mode", "smart"]:
        ...
        try:
            ...
            await update.message.reply_text("Œ Invalid option.")
...
async def ask_buy_tp(update, context, mint, amount, portfolio_manager=None, user_manager=None):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("Œ Cancel", callback_data="delete_msg")]
...
async def _execute_manual_buy(update, context, user_manager, portfolio_manager, mint, amount, tp=50.0, sl=None):
    ...
    if not token_info:
        ...
        if update.callback_query:
            ...
            await update.callback_query.message.edit_text("Œ Failed to fetch latest price. Try again.")


        else:


            await update.message.reply_text("Œ Failed to fetch latest price. Try again.")
...
async def closeposition_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, 
    ...
    if not portfolio or not portfolio.get("positions"):
        ...
        await update.message.reply_text("Œ No open positions.")
    ...
    if not position_key:
        ...
        await update.message.reply_text(f"Œ Position {symbol} not found.")
...
async def set_auto_min_prob_discovery_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):


    """Set minimum probability for Discovery auto-trades."""
    ...
    if not args:
        ...
        current_val = user_manager.get_user_prefs(chat_id).get("auto_min_prob_discovery", 0.0)


        await update.message.reply_html(


            f"🤖 <b>Min Discovery Trade Probability</b>\n\n"


            f"<b>Current:</b> {current_val * 100:.0f}%\n\n"


            f"<b>Usage:</b> <code>/set_auto_min_prob_discovery [0-100]</code>\n"


            f"<i>Example: /set_auto_min_prob_discovery 75 (for 75%+)</i>"
    ...
    try:
        ...
        user_manager.update_user_prefs(chat_id, {"auto_min_prob_discovery": prob})


        await update.message.reply_html(f"✅ Discovery auto-trade minimum probability set to <b>{val:.0f}%</b>")
...
async def set_auto_min_prob_alpha_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE, user_manager: UserManager):


    """Set minimum probability for Alpha auto-trades."""
    ...
    if not args:
        ...
        current_val = user_manager.get_user_prefs(chat_id).get("auto_min_prob_alpha", 0.0)


        await update.message.reply_html(


            f"🤖 <b>Min Alpha Trade Probability</b>\n\n"


            f"<b>Current:</b> {current_val * 100:.0f}%\n\n"


            f"<b>Usage:</b> <code>/set_auto_min_prob_alpha [0-100]</code>\n"


            f"<i>Example: /set_auto_min_prob_alpha 85 (for 85%+)</i>"
    ...
    try:
        ...
        user_manager.update_user_prefs(chat_id, {"auto_min_prob_alpha": prob})


        await update.message.reply_html(f"✅ Alpha auto-trade minimum probability set to <b>{val:.0f}%</b>")
