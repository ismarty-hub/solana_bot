"""Reconstruct button_handler in alerts/commands.py to fix corruption and add missing handlers (v4)."""
...
if start_idx != -1:
    # Find the start of the next function (set_tp_cmd) to determine end of button_handler
    # We look for the next async def that is NOT indented (top level)
    # We specifically look for set_tp_cmd or set_alert_cmd or help_cmd as anchors
    for i in range(start_idx + 1, len(lines)):
        if "async def set_tp_cmd" in line or "async def set_alert_cmd" in line or "async def help_cmd" in line:
            end_idx = i
            break
    
    if end_idx == -1:
        print("Could not find specific next function. Looking for any top-level function.")
        # Fallback: look for any async def that is not indented
        for i in range(start_idx + 1, len(lines)):
    ...
    if end_idx == -1:
        end_idx = len(lines)
        print("Warning: Could not find ANY next function. Replacing until end of file.")
    ...
    print("Successfully reconstructed button_handler (v4)!")
else:
